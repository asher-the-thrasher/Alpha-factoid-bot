...
class FactoidsExecution(commands.Cog):  



    @commands.command(hidden=True)
    async def vs2019(self,message,*,extra_text=""):
    ...
    async def usb(self,message,*,extra_text=""):
      factoid = """USB problems? Use the [USBView](https://r1ch.net/blog/managing-usb-bandwidth-with-usbview) program to identify which hubs your devices are connected to. Move bandwidth-hungry devices like webcams and capture cards to separate hubs by switching their USB ports."""
      await FactoidsExecution.execute(self, message,factoid,extra_text)
...
def setup(client):
  client.add_cog(FactoidsExecution(client))